...
{#snippet b(TDU,i)}
	<li class="transaction-item-{i + 1}" key={i}>
		...
		<span>
			{#if TDU.transactionType == 'sent'}
			<p style="color:var(--primary-red-500)">- {formatCurrency(TDU.amount)}</p>
			{:else}
			<p style="color:var(--primary-green-500)">{formatCurrency(TDU.amount)}</p>
			{/if}
			<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M10 16.75C9.80802 16.75 9.61599 16.6771 9.46999 16.5301C9.17699 16.2371 9.17699 15.762 9.46999 15.469L12.94 11.999L9.46999 8.52908C9.17699 8.23608 9.17699 7.76104 9.46999 7.46804C9.76299 7.17504 10.238 7.17504 10.531 7.46804L14.531 11.468C14.824 11.761 14.824 12.2361 14.531 12.5291L10.531 16.5291C10.384 16.6771 10.192 16.75 10 16.75Z" fill="#212121"/>
			</svg>
			<a class="transaction-route" href="/transactions/{TDU.slug}" aria-label="View details for transaction {TDU.id}"></a>
		</span>
	</li>
{/snippet}
